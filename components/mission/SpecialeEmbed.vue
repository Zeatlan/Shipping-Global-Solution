<template>
  <div class="speciale-embed">
    ...
    <nuxt-link v-if="editable" class="top-button speciale-edit" :to="`/admin/edit/speciale/${missionId}`">
      <Font-awesome-icon :icon="['fas', 'tools']" />
    </nuxt-link>
    <div v-if="editable" class="top-button speciale-delete" @click="deleteMission">
    ...
    <nuxt-link :to="'/missions/speciale/' + missionId" class="speciale-embed__button">Détails de la mission</nuxt-link>
  </div>
</template>
...
export default {
  ...
  methods: {
    dateFormat(date) {
      return date.toDate().toLocaleDateString('fr-FR', { month: 'numeric', year: 'numeric', day: 'numeric', hour: '2-digit', minute:'2-digit'}).replace(',', ' à').replace(':', 'h');
    },
    deleteMission() {
      if(window.confirm(`Êtes-vous sûr de vouloir supprimer la mission "${this.mission.name}"" ?`)) {
        ...
        this.$fire.storage.ref().child(`missions/speciales/${this.missionId}`).listAll().then(files => {
          files.items.forEach(item => {
            item.delete();
          })
        });

        this.$store.dispatch('sendNotif', {type: 'success', message: 'Mission supprimée avec succès.'})
